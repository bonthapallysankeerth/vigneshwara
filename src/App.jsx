...
function App() {
  ...
  const save = (key, item) => { if (!isAdmin) { denyChanges(); return } return run(async () => {
    ...
    const payload = key === 'chandha' ? { person_name: item.person, mobile: item.mobile, amount: item.amount, status: item.status, date: item.date, note: item.note } : key === 'expenses' ? { title: item.title, amount: item.amount, category: item.category, note: item.note, spent_by: item.spentById, date: item.date } : key === 'events' ? { day_number: item.day_number, title: item.title, description: item.description, date: item.date, location: item.location, status: 'Planned' } : key === 'members' ? { name: item.name, role: item.role || 'Team Member', team_name: item.team_name || null, photo_url: item.photo_url || null, photo_position: item.photo_position || 'center' } : key === 'bookings' ? { person_name: item.person_name, purpose_name: item.purpose_name, mobile: item.mobile, full_payment: item.full_payment, advance_paid: item.advance_paid } : { name: item.name, item: item.item, amount: item.amount, status: item.status, date: item.date, note: item.note }
    await insertRecord(table, payload, user.id)
    setModal(null)
  }) }
  const editMember = item => { if (!isAdmin) { denyChanges(); return } return run(async () => {
    await updateRecord('team_members', item.id, { name: item.name, role: item.role || 'Team Member', team_name: item.team_name || null, photo_url: item.photo_url || null, photo_position: item.photo_position || 'center' })
