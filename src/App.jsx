...
const callNumber = value => { const digits = (value || '').replace(/\D/g, ''); if (digits) window.location.href = `tel:${digits}` }
...
function App() {
  ...
  const [pendingChandhaOnly, setPendingChandhaOnly] = useState(false)
  ...
  const accountAdmin = data.accountProfiles.find(profile => profile.role === 'admin')
  const accountYouth = data.accountProfiles.find(profile => profile.role === 'youth')
  const adminEmail = user?.user_metadata?.admin_email || accountAdmin?.admin_email || (isAdmin ? user?.email : '')
  const youthEmail = user?.user_metadata?.youth_email || accountYouth?.youth_email || (!isAdmin ? user?.email : '')
  ...
  const totals = useMemo(() => {
    ...
    const loans = data.loans.reduce((total, row) => total + Number(row.amount || 0), 0)
    const repayments = data.loanRepayments.reduce((total, row) => total + Number(row.amount || 0), 0)
    const revenue = sum(data.chandha, 'Received') + sum(data.sponsors, 'Received') + loans
    const pending = sum(data.chandha, 'Pending') + sum(data.sponsors, 'Pending')
    const totalExpenses = expenses + repayments
    return { revenue, expenses: totalExpenses, repayments, pending, balance: revenue - totalExpenses, budgetLeft: revenue - totalExpenses, utilization: data.budget ? Math.round(totalExpenses / data.budget * 100) : 0 }
  }, [data])
  ...
  const save = (key, item) => { if (!isAdmin) { denyChanges(); return } return run(async () => {
    if (key === 'loan_repayments') {
      const loan = data.loans.find(row => row.id === item.loan_id)
      const alreadyPaid = data.loanRepayments.filter(row => row.loan_id === item.loan_id).reduce((sum, row) => sum + Number(row.amount || 0), 0)
      if (!loan || alreadyPaid + Number(item.amount) > Number(loan.amount)) throw new Error('Repayment cannot be greater than the remaining loan amount.')
    }
    const table = key === 'events' ? 'festival_programs' : key === 'members' ? 'team_members' : key
    const payload = key === 'chandha' ? { person_name: item.person, mobile: item.mobile, amount: item.amount, status: item.status, date: item.date, note: item.note } : key === 'expenses' ? { title: item.title, amount: item.amount, category: item.category, note: item.note, spent_by: item.spentById, date: item.date } : key === 'events' ? { day_number: item.day_number || 1, title: item.title, description: item.description, date: item.date, status: 'Planned' } : key === 'members' ? { name: item.name, role: 'Team Member' } : key === 'bookings' ? { person_name: item.person_name, purpose_name: item.purpose_name, mobile: item.mobile, full_payment: item.full_payment, advance_paid: item.advance_paid } : key === 'loans' ? { lender_name: item.lender_name, amount: Number(item.amount), date: item.date, note: item.note } : key === 'loan_repayments' ? { loan_id: item.loan_id, amount: Number(item.amount), date: item.date, note: item.note } : { name: item.name, item: item.item, amount: item.amount, status: item.status, date: item.date, note: item.note }
  ...
  const editBooking = item => { if (!isAdmin) { denyChanges(); return } return run(async () => {
    await updateRecord('bookings', item.id, { person_name: item.person_name, purpose_name: item.purpose_name, mobile: item.mobile, full_payment: Number(item.full_payment), advance_paid: Number(item.advance_paid) }, accountId)
    setEditingBooking(null)
  }) }
  const editEvent = item => { if (!isAdmin) { denyChanges(); return } return run(async () => {
      await updateRecord('festival_programs', item.id, { day_number: item.day_number || 1, title: item.title, description: item.description, date: item.date, status: 'Planned' }, accountId)
  ...
  const nav = ['Dashboard', 'Chandha', 'Expenses', 'Sponsors', 'Loans', 'Team Members', 'Bookings', 'Budget', 'Festival Program', 'Records / Reports', 'Profile']
  return <div className="app-shell">
    ...
    <main className="main"><header><button className="hamburger" onClick={() => setMenu(!menu)}>☰</button><div className="crumb">{youthName} <b>/</b> {active}</div><div className="header-actions"><span className="sync">● {syncState}</span><div className="header-avatar">{isAdmin ? 'AD' : 'YU'}</div></div></header>
      <div className="content"><div className="page-heading"><div><p className="eyebrow">{youthName.toUpperCase()} <span>•</span> SHARED DATABASE</p><h1>{active === 'Dashboard' ? `Hi ${youthName}` : active}</h1><p className="subheading">{active === 'Dashboard' ? 'Here’s the financial pulse of your festival.' : 'Keep every detail accounted for, together.'}</p></div>{active === 'Chandha' ? <div className="heading-actions"><button className="secondary" onClick={() => isAdmin ? setModal('ChandhaCollection') : denyChanges()}>＋ Add chanda</button>{pendingChandhaOnly ? <button type="button" className="secondary back-chandha-button" onClick={() => setPendingChandhaOnly(false)}>← Back to all chanda</button> : <button type="button" className="pending-chandha-card" aria-label="Show pending chanda" aria-pressed="false" onClick={() => setPendingChandhaOnly(true)}><span>Pending chanda</span></button>}<button className="primary" onClick={() => isAdmin ? setModal('Income') : denyChanges()}>＋ Add income</button></div> : ['Expenses', 'Sponsors', 'Bookings'].includes(active) && <button className="primary" onClick={() => isAdmin ? setModal(active) : denyChanges()}>＋ Add {active === 'Expenses' ? 'expense' : active === 'Bookings' ? 'booking' : 'sponsor'}</button>}</div>
        {(error || operationError) && <p className="login-error">{operationError || error}</p>}
        {loading ? <section className="panel loading">Loading {active}...</section> : <Page active={active} data={data} totals={totals} query={query} setQuery={setQuery} setActive={setActive} receive={receive} remove={remove} editSponsor={setEditingSponsor} editRecord={setEditingRecord} editBooking={setEditingBooking} editEvent={setEditingEvent} pendingChandhaOnly={pendingChandhaOnly} setPendingChandhaOnly={setPendingChandhaOnly} setBudget={setBudget} isAdmin={isAdmin} user={user} youthName={youthName} adminEmail={adminEmail} youthEmail={youthEmail} openEvent={() => isAdmin ? setModal('Event') : denyChanges()} openMember={() => isAdmin ? setModal('Member') : denyChanges()} editMember={member => isAdmin ? setEditingMember(member) : denyChanges()} openLoan={() => isAdmin ? setModal('Loan') : denyChanges()} openRepayment={loan => isAdmin ? setModal({ type: 'LoanRepayment', loan }) : denyChanges()} />}
      </div><footer className="app-footer">Any queries related to application contact - +91 9000278784 , +91 9515531463</footer>
    </main>
      {modal === 'ChandhaCollection' && <ChandhaForm mode="collection" onClose={() => setModal(null)} onSave={save} />}{modal === 'Income' && <ChandhaForm mode="income" onClose={() => setModal(null)} onSave={save} />}{modal === 'Event' && <EventForm onClose={() => setModal(null)} onSave={save} />}{modal === 'Member' && <MemberForm onClose={() => setModal(null)} onSave={save} saving={saving} />}{modal === 'Bookings' && <BookingForm onClose={() => setModal(null)} onSave={save} />}{modal === 'Loan' && <LoanForm onClose={() => setModal(null)} onSave={save} />}{modal && !['ChandhaCollection', 'Income', 'Event', 'Member', 'Bookings', 'Loan'].includes(modal) && <RecordForm type={modal} members={data.members} onClose={() => setModal(null)} onSave={save} />}{editingSponsor && <SponsorEditForm sponsor={editingSponsor} onClose={() => setEditingSponsor(null)} onSave={editSponsor} />}{editingRecord && <RecordEditForm record={editingRecord} members={data.members} youthName={youthName} onClose={() => setEditingRecord(null)} onSave={editRecord} />}{editingBooking && <BookingForm booking={editingBooking} onClose={() => setEditingBooking(null)} onSave={editBooking} />}{editingEvent && <EventForm event={editingEvent} onClose={() => setModal(null)} onSave={save} />}{editingMember && <MemberEditForm member={editingMember} onClose={() => setEditingMember(null)} onSave={editMember} saving={saving} />}{toast && <div className="toast">✓ &nbsp;{toast}</div>}
  </div>
}
...
function Page({ active, data, totals, query, setQuery, setActive, receive, remove, editSponsor, editRecord, editBooking, editEvent, pendingChandhaOnly, isAdmin, user, youthName, adminEmail, youthEmail, openEvent, openMember, editMember, openLoan, openRepayment }) { if (active === 'Dashboard') return <><Dashboard data={data} totals={totals} setActive={setActive} /><PendingList expenses={data.expenses} chandha={data.chandha} receive={receive} /></>; if (['Chandha', 'Expenses', 'Sponsors'].includes(active)) return active === 'Chandha' ? <><ChandaSummary rows={data.chandha} /><Ledger title={active} type={active.toLowerCase()} rows={data[active.toLowerCase()]} query={query} setQuery={setQuery} receive={receive} remove={remove} editSponsor={editSponsor} editRecord={editRecord} pendingOnly={pendingChandhaOnly} /></> : <Ledger title={active} type={active.toLowerCase()} rows={data[active.toLowerCase()]} query={query} setQuery={setQuery} receive={receive} remove={remove} editSponsor={editSponsor} editRecord={editRecord} pendingOnly={false} />; if (active === 'Loans') return <LoansPage loans={data.loans} repayments={data.loanRepayments} openLoan={openLoan} openRepayment={openRepayment} isAdmin={isAdmin} remove={remove} />; if (active === 'Team Members') return <Team data={data} openMember={openMember} editMember={editMember} remove={remove} />; if (active === 'Bookings') return <Bookings rows={data.bookings} editBooking={editBooking} remove={remove} />; if (active === 'Budget') return <Budget totals={totals} />; if (active === 'Festival Program') return <Program events={data.events} openEvent={openEvent} editEvent={editEvent} remove={remove} />; if (active === 'Profile') return <Profile user={user} youthName={youthName} adminEmail={adminEmail} youthEmail={youthEmail} isAdmin={isAdmin} />; return <Reports data={data} totals={totals} /> }
...
function PendingList({ expenses, chandha, receive }) { const pending = [...expenses.filter(item => item.status === 'Pending').map(item => ({ ...item, label: item.title, type: 'Expense' })), ...chandha.filter(item => item.status === 'Pending').map(item => ({ ...item, label: item.person_name, type: 'Chandha' }))]; return <section className="panel pending-panel"><div className="panel-head"><div><h2>Pending payments</h2><p>Expenses and Chandha still awaiting payment.</p></div></div>{pending.length ? pending.map(item => <div className="mini-row" key={`${item.type}-${item.id}`}><span>{item.label}<small>{item.type}{item.date ? ` · ${item.date}` : ''}</small></span><strong className="pending-amount">{money(item.amount)}</strong>{item.type === 'Chandha' && <>{item.mobile && <button className="call-button" onClick={() => callNumber(item.mobile)}>Call</button>}<button className="receive" onClick={() => receive('chandha', item.id)}>Receive</button></>}</div>) : <p className="empty">No pending payments.</p>}</section> }
function ChandaSummary({ rows }) { const total = rows.reduce((sum, row) => sum + Number(row.amount || 0), 0); const received = rows.filter(row => row.status === 'Received').reduce((sum, row) => sum + Number(row.amount || 0), 0); const pending = rows.filter(row => row.status === 'Pending').reduce((sum, row) => sum + Number(row.amount || 0), 0); return <section className="chanda-summary" aria-label="Chanda amounts"><div><span>Total chanda amount</span><strong>{money(total)}</strong></div><div><span>Received amount</span><strong>{money(received)}</strong></div><div><span>Pending amount</span><strong>{money(pending)}</strong></div></section> }
function Ledger({ title, type, rows, query, setQuery, receive, remove, editSponsor, editRecord, pendingOnly }) {
  const filtered = rows.filter(item => (!pendingOnly || item.status === 'Pending') && (item.person_name || item.name || item.title || '').toLowerCase().includes(query.toLowerCase()))
  const rowActions = item => <span className="row-actions">{['chandha', 'expenses'].includes(type) && <button className="row-edit" aria-label="Edit record" title="Edit record" onClick={() => editRecord({ ...item, type })}>✎</button>}{type === 'sponsors' && <button className="row-edit" aria-label="Edit sponsor" title="Edit sponsor" onClick={() => editSponsor(item)}>✎</button>}{type === 'chandha' && item.status === 'Pending' && <button className="call-button" aria-label={`Call ${item.person_name}`} title={item.mobile ? 'Call contributor' : 'Mobile number not available'} disabled={!item.mobile} onClick={() => callNumber(item.mobile)}>Call</button>}{item.status === 'Pending' && <button className="receive" onClick={() => receive(type, item.id)}>Receive</button>}<button className="row-delete" aria-label="Delete record" title="Delete record" onClick={() => remove(type, item.id)}>⌫</button></span>
  return <>
    <section className="panel ledger"><div className="table-tools"><h2>{title}</h2><label className="search">⌕<input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search records" /></label></div><div className="table-wrap"><table><thead><tr><th>{type === 'expenses' ? 'Expense' : type === 'chandha' ? 'Contributor' : 'Sponsor'}</th><th>{type === 'expenses' ? 'Category' : type === 'chandha' ? 'Mobile' : 'Sponsored item'}</th><th>Amount</th><th>{type === 'expenses' ? 'Spent by' : 'Status'}</th><th>Date</th><th /></tr></thead><tbody>{filtered.map(item => <tr key={item.id}><td><b>{item.person_name || item.name || item.title}</b><small>{item.note}</small></td><td>{item.category || item.mobile || item.item}</td><td><strong>{money(item.amount)}</strong></td><td>{type === 'expenses' ? item.spentBy : <span className={`status ${item.status.toLowerCase()}`}>{item.status}</span>}</td><td>{item.date}</td><td>{rowActions(item)}</td></tr>)}</tbody></table></div>{!filtered.length && <p className="empty">No records found.</p>}</section>
  </>
}
function RecordForm({ type, members, onClose, onSave }) { const [form, setForm] = useState({ status: 'Received', date: new Date().toISOString().slice(0, 10) }); if (typeof type === 'object' && type?.type === 'LoanRepayment') return <LoanRepaymentForm loan={type.loan} onClose={onClose} onSave={onSave} />; const update = event => setForm({ ...form, [event.target.name]: event.target.value }); const expense = type === 'Expenses'; const key = expense ? 'expenses' : type.toLowerCase(); const submit = event => { event.preventDefault(); onSave(key, expense ? { ...form, amount: Number(form.amount), spentById: form.spentBy } : { ...form, person: form.person, name: form.name, amount: Number(form.amount) }) }; return <div className="modal-backdrop"><form className="modal" onSubmit={submit}><div className="modal-head"><div><p className="eyebrow">NEW RECORD</p><h2>Add {type.toLowerCase()}</h2></div><button type="button" onClick={onClose}>×</button></div>{expense ? <><input autoFocus required name="title" onChange={update} placeholder="Expense name / item" /><div className="form-row"><input required type="number" name="amount" onChange={update} placeholder="Amount (₹)" /><select required name="category" defaultValue="" onChange={update}><option value="" disabled>Category</option>{categories.map(category => <option key={category}>{category}</option>)}</select></div><select required name="spentBy" defaultValue="" onChange={update}><option value="" disabled>Spent by</option>{members.map(member => <option value={member.id} key={member.id}>{member.name}</option>)}</select></> : <><input autoFocus required name={key === 'chandha' ? 'person' : 'name'} onChange={update} placeholder={key === 'chandha' ? 'Contributor name' : 'Sponsor name'} /><input required type="number" name="amount" onChange={update} placeholder="Amount (₹)" /><input name={key === 'chandha' ? 'mobile' : 'item'} onChange={update} placeholder={key === 'chandha' ? 'Mobile' : 'Sponsored item'} /><select name="status" onChange={update} defaultValue="Received"><option>Received</option><option>Pending</option></select></>}<button className="primary" type="submit">Save record</button></form></div> }

function LoanForm({ onClose, onSave }) { const [form, setForm] = useState({ date: new Date().toISOString().slice(0, 10) }); const update = event => setForm({ ...form, [event.target.name]: event.target.value }); return <div className="modal-backdrop"><form className="modal" onSubmit={event => { event.preventDefault(); onSave('loans', form) }}><div className="modal-head"><div><p className="eyebrow">NEW LOAN</p><h2>Add loan</h2></div><button type="button" onClick={onClose}>×</button></div><input autoFocus required name="lender_name" onChange={update} placeholder="Loan name / lender" /><input required type="number" min="0.01" step="0.01" name="amount" onChange={update} placeholder="Loan amount (₹)" /><input required type="date" name="date" value={form.date} onChange={update} /><input name="note" onChange={update} placeholder="Note (optional)" /><div className="form-actions"><button className="secondary" type="button" onClick={onClose}>Cancel</button><button className="primary" type="submit">Submit loan</button></div></form></div> }

function LoanRepaymentForm({ loan, onClose, onSave }) { const [form, setForm] = useState({ loan_id: loan.id, date: new Date().toISOString().slice(0, 10) }); const update = event => setForm({ ...form, [event.target.name]: event.target.value }); return <div className="modal-backdrop"><form className="modal" onSubmit={event => { event.preventDefault(); onSave('loan_repayments', form) }}><div className="modal-head"><div><p className="eyebrow">LOAN REPAYMENT</p><h2>Repay {loan.lender_name}</h2></div><button type="button" onClick={onClose}>×</button></div><input autoFocus required type="number" min="0.01" step="0.01" name="amount" onChange={update} placeholder="Repayment amount (₹)" /><input required type="date" name="date" value={form.date} onChange={update} /><input name="note" onChange={update} placeholder="Note (optional)" /><div className="form-actions"><button className="secondary" type="button" onClick={onClose}>Cancel</button><button className="primary" type="submit">Submit repayment</button></div></form></div> }

function LoansPage({ loans, repayments, openLoan, openRepayment, isAdmin, remove }) { const paid = loan => repayments.filter(item => item.loan_id === loan.id).reduce((sum, item) => sum + Number(item.amount || 0), 0); return <section className="panel loans-page"><div className="panel-head"><div><h2>Loans</h2><p>Loan amounts increase revenue; repayments are counted as expenses.</p></div>{isAdmin && <button className="primary" onClick={openLoan}>＋ Add loan</button>}</div>{loans.length ? loans.map(loan => { const repaid = paid(loan); const remaining = Math.max(Number(loan.amount) - repaid, 0); return <div className="loan-row" key={loan.id}><div><b>{loan.lender_name}</b><small>{loan.date}{loan.note ? ` · ${loan.note}` : ''}</small></div><span>Loan<br /><strong>{money(loan.amount)}</strong></span><span>Repaid<br /><strong>{money(repaid)}</strong></span><span>Remaining<br /><strong className={remaining ? 'pending-amount' : 'green-text'}>{money(remaining)}</strong></span>{isAdmin && remaining > 0 && <button type="button" className="loan-repay-button" onClick={() => openRepayment(loan)}>＋ Repay</button>}<button className="row-delete" aria-label={`Delete loan from ${loan.lender_name}`} onClick={() => remove('loans', loan.id)}>⌫</button></div> }) : <p className="empty">No loans recorded.</p>}</section> }
...
function Program({ events, openEvent, editEvent, remove }) { return <section className="panel events-list"><div className="panel-head"><div><h2>Festival program</h2><p>Shared schedule from Supabase.</p></div><button className="primary" onClick={openEvent}>＋ Add program</button></div>{events.length ? events.map(event => <div className="program-row" key={event.id}><div className="program-number">{event.date || '—'}</div><div><p>EVENT</p><h3>{event.title}</h3><span>{event.description}</span></div><span className="row-actions"><button className="row-edit" aria-label="Edit program" title="Edit program" onClick={() => editEvent(event)}>✎</button><button className="row-delete" aria-label="Delete program" title="Delete program" onClick={() => remove('festival_programs', event.id)}>⌫</button></span></div>) : <p className="empty">No events added yet. Add a program when the schedule is ready.</p>}</section> }
...
function Profile({ user, youthName, adminEmail, youthEmail, isAdmin }) { return <section className="profile-page"><div className="panel profile-header"><div className="profile-avatar">{isAdmin ? 'AD' : 'YU'}</div><div><p className="eyebrow">ACCOUNT PROFILE</p><h2>{youthName}</h2><p>{isAdmin ? 'Admin account' : 'Youth account'} · Signed in as {user.email}</p></div></div><div className="profile-details"><div className="panel profile-detail"><span>Admin mail id</span><strong>{adminEmail || 'Not available'}</strong></div><div className="panel profile-detail"><span>Youth mail id</span><strong>{youthEmail || 'Not available'}</strong></div><div className="panel profile-detail"><span>Team Name</span><strong>{youthName}</strong></div><div className="panel profile-detail"><span>Current account</span><strong>{isAdmin ? 'Admin' : 'Youth'}</strong><small>{user.email}</small></div></div></section> }

function EventForm({ event, onClose, onSave }) {
  const editing = Boolean(event)
  const [form, setForm] = useState(event || { date: new Date().toISOString().slice(0, 10) })
  const update = event => setForm({ ...form, [event.target.name]: event.target.value })
  const submit = submitEvent => { submitEvent.preventDefault(); onSave(editing ? { ...form, id: event.id } : 'events', form) }
  return <div className="modal-backdrop"><form className="modal" onSubmit={submit}><div className="modal-head"><div><p className="eyebrow">{editing ? 'EDIT EVENT' : 'NEW EVENT'}</p><h2>{editing ? 'Update program' : 'Add program'}</h2></div><button type="button" onClick={onClose}>×</button></div><div className="form-row"><input autoFocus required name="title" value={form.title || ''} onChange={update} placeholder="Event name" /><input required type="date" name="date" value={form.date || ''} onChange={update} /></div><input name="description" value={form.description || ''} onChange={update} placeholder="Description" /><div className="form-actions"><button className="secondary" type="button" onClick={onClose}>Cancel</button><button className="primary" type="submit">{editing ? 'Save changes' : 'Submit'}</button></div></form></div>
}
...
function ChandhaForm({ mode, onClose, onSave }) {
  ...
  const update = event => setForm({ ...form, [event.target.name]: event.target.name === 'mobile' ? event.target.value.replace(/\D/g, '').slice(0, 10) : event.target.value })
  const getMessageDetails = () => {
    const digits = (form.mobile || '').replace(/\D/g, '')
    const phone = digits.length === 10 ? `91${digits}` : digits
    if (!phone || !form.person || !form.amount || form.status !== 'Received') return null
  ...
  const sendWhatsApp = () => {
    ...
    if (!details) return
    const whatsappUrl = `https://api.whatsapp.com/send?phone=${details.phone}&text=${encodeURIComponent(details.message)}`
  ...
  const sendNormalMessage = () => {
    const details = getMessageDetails()
    if (!details) return
    window.location.href = `sms:${details.phone}?body=${encodeURIComponent(details.message)}`
  }
  const submit = event => {
    event.preventDefault()
  ...
  return <div className="modal-backdrop"><form className="modal" onSubmit={submit}><div className="modal-head"><div><p className="eyebrow">NEW RECORD</p><h2>Add {collection ? 'chanda' : 'income'}</h2></div><button type="button" onClick={onClose}>×</button></div>{collection ? <><input autoFocus required name="person" onChange={update} placeholder="Contributor name" /><input name="mobile" onChange={update} placeholder="Mobile number (optional)" /><input required type="number" name="amount" onChange={update} placeholder="Amount contributed (₹)" /><select name="status" onChange={update} defaultValue="Received"><option>Received</option><option>Pending</option></select>{form.status === 'Received' && <div className="message-actions"><button className="secondary" type="button" onClick={sendWhatsApp}>Send WhatsApp message</button><button className="secondary" type="button" onClick={sendNormalMessage}>Send normal message</button></div>}</> : <><input autoFocus required name="source" onChange={update} placeholder="Income name / source" /><input required type="number" name="amount" onChange={update} placeholder="Amount received (₹)" /><input required name="note" onChange={update} placeholder="Note" /><select name="status" onChange={update} defaultValue="Received"><option>Received</option><option>Pending</option></select></>}<div className="form-actions"><button className="secondary" type="button" onClick={onClose}>Cancel</button><button className="primary" type="submit">Submit</button></div></form></div>
}
...
function Bookings({ rows, editBooking, remove }) { return <section className="panel ledger"><div className="table-tools"><h2>Bookings</h2></div><div className="table-wrap"><table><thead><tr><th>Person</th><th>Purpose</th><th>Mobile</th><th>Full payment</th><th>Advance</th><th>Balance</th><th /></tr></thead><tbody>{rows.map(row => <tr key={row.id}><td><b>{row.person_name}</b></td><td>{row.purpose_name}</td><td>{row.mobile || 'NA'}</td><td>{money(row.full_payment)}</td><td>{money(row.advance_paid)}</td><td><strong>{money(row.balance_amount)}</strong></td><td><span className="row-actions"><button className="row-edit" aria-label="Edit booking" title="Edit booking" onClick={() => editBooking(row)}>✎</button><button className="row-delete" aria-label="Delete booking" title="Delete booking" onClick={() => remove('bookings', row.id)}>⌫</button></span></td></tr>)}</tbody></table></div>{!rows.length && <p className="empty">No bookings added yet.</p>}</section> }

function BookingForm({ booking, onClose, onSave }) { const editing = Boolean(booking); const [form, setForm] = useState(booking || { full_payment: '', advance_paid: '' }); const update = event => setForm({ ...form, [event.target.name]: event.target.name === 'mobile' ? event.target.value.replace(/\D/g, '').slice(0, 10) : event.target.value }); const balance = Number(form.full_payment || 0) - Number(form.advance_paid || 0); const submit = event => { event.preventDefault(); const payload = { ...form, full_payment: Number(form.full_payment), advance_paid: Number(form.advance_paid) }; onSave(editing ? { ...payload, id: booking.id } : 'bookings', editing ? payload : payload) }; return <div className="modal-backdrop"><form className="modal" onSubmit={submit}><div className="modal-head"><div><p className="eyebrow">{editing ? 'EDIT BOOKING' : 'NEW BOOKING'}</p><h2>{editing ? 'Update booking' : 'Add booking'}</h2></div><button type="button" onClick={onClose}>×</button></div><input autoFocus required name="person_name" value={form.person_name || ''} onChange={update} placeholder="Person name" /><input required name="purpose_name" value={form.purpose_name || ''} onChange={update} placeholder="Purpose name" /><input maxLength="10" pattern="[0-9]{10}" name="mobile" value={form.mobile || ''} onChange={update} placeholder="Mobile number (optional)" /><div className="form-row"><input required min="0" type="number" name="full_payment" value={form.full_payment || ''} onChange={update} placeholder="Full payment (₹)" /><input required min="0" type="number" name="advance_paid" value={form.advance_paid || ''} onChange={update} placeholder="Advance paid (₹)" /></div><p className="balance-preview">Balance amount: <strong>{money(balance)}</strong></p><div className="form-actions"><button className="secondary" type="button" onClick={onClose}>Cancel</button><button className="primary" type="submit">{editing ? 'Save changes' : 'Submit'}</button></div></form></div> }

function MessageActions({ person, mobile, amount, status, teamName }) {
  const sendMessage = scheme => {
    const digits = (mobile || '').replace(/\D/g, '')
    const phone = digits.length === 10 ? `91${digits}` : digits
    if (!phone || !person || !amount || status !== 'Received') return
    const message = `Hi ${person}, Happy Vinayaka Chaturthi! Thank you for contributing ₹${amount}. Thank you from ${teamName || 'Youth Association'}.`
    const url = scheme === 'whatsapp'
  ...
  return <div className="message-actions"><button className="secondary" type="button" onClick={() => sendMessage('whatsapp')}>Send WhatsApp message</button><button className="secondary" type="button" onClick={() => sendMessage('sms')}>Send normal message</button></div>
}

function RecordEditForm({ record, members, youthName, onClose, onSave }) { const expense = record.type === 'expenses'; const [form, setForm] = useState({ ...record, person: record.person_name, spentById: record.spent_by }); const update = event => setForm({ ...form, [event.target.name]: event.target.name === 'mobile' ? event.target.value.replace(/\D/g, '').slice(0, 10) : event.target.value }); const submit = event => { event.preventDefault(); onSave({ ...form, person: form.person, amount: Number(form.amount) }); }; return <div className="modal-backdrop"><form className="modal" onSubmit={submit}><div className="modal-head"><div><p className="eyebrow">EDIT RECORD</p><h2>Update {expense ? 'expense' : 'chandha'}</h2></div><button type="button" onClick={onClose}>×</button></div>{expense ? <><input required name="title" value={form.title || ''} onChange={update} placeholder="Expense name / item" /><div className="form-row"><input required type="number" name="amount" value={form.amount || ''} onChange={update} placeholder="Amount (₹)" /><select required name="category" value={form.category || ''} onChange={update}>{categories.map(category => <option key={category}>{category}</option>)}</select></div><select required name="spentById" value={form.spentById || ''} onChange={update}><option value="" disabled>Spent by</option>{members.map(member => <option value={member.id} key={member.id}>{member.name}</option>)}</select></> : <><input required name="person" value={form.person || ''} onChange={update} placeholder="Contributor name" /><input maxLength="10" pattern="[0-9]{10}" name="mobile" value={form.mobile || ''} onChange={update} placeholder="Mobile (optional)" /><input required type="number" name="amount" value={form.amount || ''} onChange={update} placeholder="Amount (₹)" /><select name="status" value={form.status || 'Received'} onChange={update}><option>Received</option><option>Pending</option></select><MessageActions person={form.person} mobile={form.mobile} amount={form.amount} status={form.status} teamName={youthName} /></>}<div className="form-actions"><button className="secondary" type="button" onClick={onClose}>Cancel</button><button className="primary" type="submit">Save changes</button></div></form></div> }
