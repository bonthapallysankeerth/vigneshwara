  ...
  return <div className="app-shell">
    ...
    <main className="main"><header><button className="hamburger" onClick={() => setMenu(!menu)}>☰</button><div className="crumb">Festival Management <b>/</b> {active}</div><div className="header-actions"><span className="sync">● {syncState}</span><div className="header-avatar">VT</div></div></header>
      <div className="content"><div className="page-heading"><div><p className="eyebrow">FESTIVAL MANAGEMENT <span>•</span> SHARED DATABASE</p><h1>{active === 'Dashboard' ? 'Hi Vigneshwara Team' : active}</h1><p className="subheading">{active === 'Dashboard' ? 'Here’s the financial pulse of your festival.' : 'Keep every detail accounted for, together.'}</p></div>{active === 'Chandha' ? <div className="heading-actions"><button className="secondary" onClick={() => setModal('ChandhaCollection')}>＋ Add chanda</button><button className="primary" onClick={() => setModal('Income')}>＋ Add income</button></div> : ['Expenses', 'Sponsors'].includes(active) && <button className="primary" onClick={() => setModal(active)}>＋ Add {active === 'Expenses' ? 'expense' : 'sponsor'}</button>}</div>
        ...
        {loading ? <section className="panel loading">Loading {active}...</section> : <Page active={active} data={data} totals={totals} query={query} setQuery={setQuery} setActive={setActive} receive={receive} remove={remove} editSponsor={setEditingSponsor} setBudget={setBudget} openEvent={() => setModal('Event')} openMember={() => setModal('Member')} />}
      </div>
    </main>
      {modal === 'ChandhaCollection' && <ChandhaForm mode="collection" onClose={() => setModal(null)} onSave={save} />}{modal === 'Income' && <ChandhaForm mode="income" onClose={() => setModal(null)} onSave={save} />}{modal === 'Event' && <EventForm onClose={() => setModal(null)} onSave={save} />}{modal === 'Member' && <MemberForm onClose={() => setModal(null)} onSave={save} />}{modal && !['ChandhaCollection', 'Income', 'Event', 'Member'].includes(modal) && <RecordForm type={modal} members={data.members} onClose={() => setModal(null)} onSave={save} />}{editingSponsor && <SponsorEditForm sponsor={editingSponsor} onClose={() => setEditingSponsor(null)} onSave={editSponsor} />}{toast && <div className="toast">✓ &nbsp;{toast}</div>}
  </div>
}
...
function Page({ active, data, totals, query, setQuery, setActive, receive, remove, editSponsor, setBudget, openEvent, openMember }) { if (active === 'Dashboard') return <><Dashboard data={data} totals={totals} setActive={setActive} /><PendingList expenses={data.expenses} chandha={data.chandha} receive={receive} /></>; if (['Chandha', 'Expenses', 'Sponsors'].includes(active)) return <Ledger title={active} type={active.toLowerCase()} rows={data[active.toLowerCase()]} query={query} setQuery={setQuery} receive={receive} remove={remove} editSponsor={editSponsor} />; if (active === 'Team Members') return <Team data={data} openMember={openMember} />; if (active === 'Budget') return <Budget data={data} totals={totals} setBudget={setBudget} />; if (active === 'Festival Program') return <Program events={data.events} openEvent={openEvent} />; return <Reports data={data} totals={totals} /> }
...
function Ledger({ title, type, rows, query, setQuery, receive, remove, editSponsor }) { const filtered = rows.filter(item => (item.person_name || item.name || item.title || '').toLowerCase().includes(query.toLowerCase())); return <section className="panel ledger"><div className="table-tools"><h2>{title}</h2><label className="search">⌕<input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search records" /></label></div><div className="table-wrap"><table><thead><tr><th>{type === 'expenses' ? 'Expense' : type === 'chandha' ? 'Contributor' : 'Sponsor'}</th><th>{type === 'expenses' ? 'Category' : type === 'chandha' ? 'Mobile' : 'Sponsored item'}</th><th>Amount</th><th>{type === 'expenses' ? 'Spent by' : 'Status'}</th><th>Date</th><th /></tr></thead><tbody>{filtered.map(item => <tr key={item.id}><td><b>{item.person_name || item.name || item.title}</b><small>{item.note}</small></td><td>{item.category || item.mobile || item.item}</td><td><strong>{money(item.amount)}</strong></td><td>{type === 'expenses' ? item.spentBy : <span className={`status ${item.status.toLowerCase()}`}>{item.status}</span>}{type === 'sponsors' && <span className="row-actions"><button className="row-edit" aria-label="Edit sponsor" title="Edit sponsor" onClick={() => editSponsor(item)}>✎</button><button className="row-delete" aria-label="Delete sponsor" title="Delete sponsor" onClick={() => remove('sponsors', item.id)}>⌫</button></span>}</td><td>{item.date}</td><td>{item.status === 'Pending' && <button className="receive" onClick={() => receive(type, item.id)}>Receive</button>}{['chandha', 'expenses'].includes(type) && <button className="delete icon-delete" aria-label="Delete record" title="Delete record" onClick={() => remove(type, item.id)}>⌫</button>}</td></tr>)}</tbody></table></div>{!filtered.length && <p className="empty">No records found.</p>}</section> }
...
function Team({ data, openMember }) { return <section className="team-page"><div className="panel-head"><div><h2>Team members</h2><p>People managing the festival.</p></div><button className="primary" onClick={openMember}>＋ Add member</button></div><div className="team-grid">{data.members.map(member => { const expenses = data.expenses.filter(item => item.spent_by === member.id); return <div className="member-card" key={member.id}><div className="member-avatar">{member.name.slice(0, 2).toUpperCase()}</div><h3>{member.name}</h3><p>{member.role}</p><div className="member-stats"><span>{expenses.length} expenses</span><strong>{money(expenses.reduce((sum, item) => sum + Number(item.amount), 0))}</strong></div></div> })}</div></section> }
...
function MemberForm({ onClose, onSave }) {
  ...
  const submit = event => { event.preventDefault(); onSave('members', { name }) }
  return <div className="modal-backdrop"><form className="modal" onSubmit={submit}><div className="modal-head"><div><p className="eyebrow">NEW MEMBER</p><h2>Add member</h2></div><button type="button" onClick={onClose}>×</button></div><input required value={name} onChange={event => setName(event.target.value)} placeholder="Member name" /><div className="form-actions"><button className="secondary" type="button" onClick={onClose}>Cancel</button><button className="primary" type="submit">Add member</button></div></form></div>
}
