...
function Page({ active, data, totals, query, setQuery, setActive, receive, remove, editSponsor, editRecord, isAdmin, user, youthName, adminEmail, youthEmail, openEvent, openMember, editMember }) { if (active === 'Dashboard') return <><Dashboard data={data} totals={totals} setActive={setActive} /><PendingList expenses={data.expenses} chandha={data.chandha} receive={receive} /></>; if (['Chandha', 'Expenses', 'Sponsors'].includes(active)) return <Ledger title={active} type={active.toLowerCase()} rows={data[active.toLowerCase()]} query={query} setQuery={setQuery} receive={receive} remove={remove} editSponsor={editSponsor} editRecord={editRecord} />; if (active === 'Team Members') return <Team data={data} openMember={openMember} editMember={editMember} remove={remove} />; if (active === 'Bookings') return <Bookings rows={data.bookings} />; if (active === 'Budget') return <Budget totals={totals} />; if (active === 'Festival Program') return <Program events={data.events} openEvent={openEvent} />; if (active === 'Profile') return <Profile user={user} youthName={youthName} adminEmail={adminEmail} youthEmail={youthEmail} isAdmin={isAdmin} />; return <Reports data={data} totals={totals} /> }
