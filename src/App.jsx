...
function App() {
  ...
  const nav = ['Dashboard', 'Chandha', 'Expenses', 'Sponsors', 'Team Members', 'Bookings', 'Budget', 'Festival Program', 'Records / Reports', 'Profile']
  return <div className="app-shell">
    ...
    <main className="main"><header><button className="hamburger" onClick={() => setMenu(!menu)}>☰</button><div className="crumb">{youthName} <b>/</b> {active}</div><div className="header-actions"><span className="sync">● {syncState}</span><div className="header-avatar">{isAdmin ? 'AD' : 'YU'}</div></div></header>
      ...
      {modal === 'ChandhaCollection' && <ChandhaForm mode="collection" onClose={() => setModal(null)} onSave={save} />}{modal === 'Income' && <ChandhaForm mode="income" onClose={() => setModal(null)} onSave={save} />}{modal === 'Event' && <EventForm onClose={() => setModal(null)} onSave={save} />}{modal === 'Member' && <MemberForm onClose={() => setModal(null)} onSave={save} saving={saving} />}{modal === 'Bookings' && <BookingForm onClose={() => setModal(null)} onSave={save} />}{modal && !['ChandhaCollection', 'Income', 'Event', 'Member', 'Bookings'].includes(modal) && <RecordForm type={modal} members={data.members} onClose={() => setModal(null)} onSave={save} />}{editingSponsor && <SponsorEditForm sponsor={editingSponsor} onClose={() => setEditingSponsor(null)} onSave={editSponsor} />}{editingRecord && <RecordEditForm record={editingRecord} members={data.members} youthName={youthName} onClose={() => setEditingRecord(null)} onSave={editRecord} />}{editingBooking && <BookingForm booking={editingBooking} onClose={() => setEditingBooking(null)} onSave={editBooking} />}{editingEvent && <EventForm event={editingEvent} onClose={() => setEditingEvent(null)} onSave={editEvent} />}{editingMember && <MemberEditForm member={editingMember} onClose={() => setEditingMember(null)} onSave={editMember} saving={saving} />}{toast && <div className="toast">✓ &nbsp;{toast}</div>}
  </div>
}
