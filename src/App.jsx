...
const members = ['Vijay Kumar', 'S Sai Kishore', 'Venu Gopal', 'Mani', 'Revanth', 'Nani Anil', 'Ibrahim', 'Ganesh', 'Naveen', 'Nithin', 'Pranay', 'Akshay', 'Sankeerth', 'Naveen', 'Varshith', 'Naveen DRC', 'Jagadish', 'Pavan', 'Vikas', 'Jithender', 'Nagu', 'Lalli']
...
function App() {
  ...
  const totals = useMemo(() => {
    ...
    const receivedSponsors = data.sponsors.filter(x => x.status === 'Received').reduce((s, x) => s + Number(x.amount), 0)
    const pendingSponsors = data.sponsors.filter(x => x.status === 'Pending').reduce((s, x) => s + Number(x.amount), 0)
...
function Ledger({ title, type, rows, query, setQuery, receive, remove }) { const filtered = rows.filter(item => (item.person || item.name || item.title).toLowerCase().includes(query.toLowerCase())); const total = rows.reduce((s, item) => s + Number(item.amount), 0); return <section className="panel ledger"><div className="ledger-summary"><div><span>Total {type === 'expenses' ? 'expenses' : type === 'chandha' ? 'chandha' : 'sponsorship'}</span><strong>{money(total)}</strong></div><div><span>Received</span><strong className="green-text">{money(rows.filter(item => type === 'expenses' || item.status === 'Received').reduce((s, item) => s + Number(item.amount), 0))}</strong></div><div><span>{type === 'expenses' ? 'Entries' : 'Pending'}</span><strong>{type === 'expenses' ? rows.length : money(rows.filter(item => item.status === 'Pending').reduce((s, item) => s + Number(item.amount), 0))}</strong></div></div><div className="table-tools"><h2>{title}</h2><label className="search">⌕<input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search records" /></label></div><div className="table-wrap"><table><thead><tr><th>{type === 'expenses' ? 'Expense' : type === 'chandha' ? 'Contributor' : 'Sponsor'}</th><th>{type === 'expenses' ? 'Category' : type === 'chandha' ? 'Mobile' : 'Sponsored item'}</th><th>Amount</th><th>{type === 'expenses' ? 'Spent by' : 'Status'}</th><th>Date</th><th /></tr></thead><tbody>{filtered.map(item => <tr key={item.id}><td><b>{item.person || item.name || item.title}</b><small>{item.note}</small></td><td>{item.category || item.mobile || item.item}</td><td><strong>{money(item.amount)}</strong></td><td>{type === 'expenses' ? item.spentBy : <span className={`status ${item.status.toLowerCase()}`}>{item.status}</span>}</td><td>{item.date}</td><td><button className="row-action">•••</button>{item.status === 'Pending' && <button className="receive" onClick={() => receive(type, item.id)}>Receive</button>}<button className="delete" onClick={() => remove(type, item.id)}>Delete</button></td></tr>)}</tbody></table></div></section> }

function RecordForm({ type, onClose, onSave }) { const [form, setForm] = useState({}); const update = e => setForm({ ...form, [e.target.name]: e.target.value }); const event = type === 'Event'; const income = type === 'Income'; const expense = type === 'Expenses'; const key = event ? 'events' : income ? 'incomes' : expense ? 'expenses' : type.toLowerCase(); const submit = e => { e.preventDefault(); const item = { ...form, id: Date.now(), amount: Number(form.amount || form.totalAmount), date: form.date || new Date().toISOString().slice(0, 10), status: form.status || 'Received' }; if (event) { item.totalAmount = Number(form.totalAmount); item.advanceAmount = Number(form.advanceAmount || 0); item.pendingAmount = item.totalAmount - item.advanceAmount; delete item.amount } onSave(key, item) }; return <div className="modal-backdrop"><form className="modal" onSubmit={submit}><div className="modal-head"><div><p className="eyebrow">NEW RECORD</p><h2>Add {type.toLowerCase()}</h2></div><button type="button" onClick={onClose}>×</button></div>{event ? <><input required name="eventName" onChange={update} placeholder="Event name (Panthul, Band, Lighting...)" /><input required name="personName" onChange={update} placeholder="Person name" /><div className="form-row"><input required type="number" name="totalAmount" onChange={update} placeholder="Total amount (₹)" /><input type="number" name="advanceAmount" onChange={update} placeholder="Advance amount (₹)" /></div><input name="note" onChange={update} placeholder="Note (optional)" /></> : income ? <><input required name="title" onChange={update} placeholder="Income name / source" /><input required type="number" name="amount" onChange={update} placeholder="Amount (₹)" /><input required name="note" onChange={update} placeholder="Note" /></> : expense ? <><input required name="title" onChange={update} placeholder="Expense name / item" /><div className="form-row"><input required type="number" name="amount" onChange={update} placeholder="Amount (₹)" /><select required name="category" onChange={update}><option value="">Category</option>{categories.map(item => <option key={item}>{item}</option>)}</select></div><select required name="spentBy" onChange={update}><option value="">Spent by team member</option>{members.map((item, index) => <option key={`${item}-${index}`}>{item}</option>)}</select><input name="note" onChange={update} placeholder="Note (optional)" /></> : <><input required name={type === 'Chandha' ? 'person' : 'name'} onChange={update} placeholder={type === 'Chandha' ? 'Person name' : 'Sponsor name'} /><div className="form-row"><input required type="number" name="amount" onChange={update} placeholder="Amount (₹)" /><select name="status" onChange={update}><option>Received</option><option>Pending</option></select></div><input name={type === 'Chandha' ? 'mobile' : 'item'} onChange={update} placeholder={type === 'Chandha' ? 'Mobile number' : 'Sponsored item'} /></>}<div className="modal-actions"><button type="button" className="secondary" onClick={onClose}>Cancel</button><button className="primary" type="submit">Submit</button></div></form></div> }

function Program({ events, openEvent }) { return <><section className="panel events-list"><div className="panel-head"><div><h2>Events list</h2><p>Track Panthul, band, lighting, tochan and vehicle costs.</p></div><button className="primary" onClick={openEvent}>＋ Add event</button></div>{events.length ? events.map(event => <div className="event-row" key={event.id}><div><b>{event.eventName}</b><small>{event.personName}</small></div><span>Total {money(event.totalAmount)}</span><span>Advance {money(event.advanceAmount)}</span><strong>Pending {money(event.pendingAmount)}</strong></div>) : <p className="empty">No events recorded yet.</p>}</section><section className="panel program"><div className="panel-head"><div><h2>10-day festival program</h2><p>Festival begins 14 September 2026.</p></div></div>{days.map((day, i) => <div className="program-row" key={day}><div className="program-number">{String(i + 1).padStart(2, '0')}</div><div><p>DAY {i + 1} · SEP {14 + i}</p><h3>{day}</h3><span>Community temple grounds · 6:00 PM - 9:00 PM</span></div></div>)}</section></> }
function Team({ data }) { return <section className="team-grid">{members.map((name, i) => { const mine = data.expenses.filter(item => item.spentBy === name); return <div className="member-card" key={`${name}-${i}`}><div className="member-avatar">{name.slice(0, 2).toUpperCase()}</div><h3>{name}</h3><p>{i === 0 ? 'President' : 'Committee member'}</p><div className="member-stats"><span>{mine.length} expenses</span><strong>{money(mine.reduce((s, item) => s + Number(item.amount), 0))}</strong></div></div> })}</section> }
function Budget({ data, totals, setData }) { return <section className="budget-page"><div className="panel budget-hero"><div><p className="eyebrow">FINANCIAL CONTROL</p><h2>Know where every rupee stands.</h2><p>Set the total budget for the 2026 celebration.</p></div><div className="big-budget"><span>Total budget</span><strong>{money(data.budget)}</strong><input type="number" value={data.budget} onChange={e => setData({ ...data, budget: Number(e.target.value) })} /></div></div><div className="stat-grid compact"><div className="stat-card cream"><span>Total revenue</span><strong>{money(totals.revenue)}</strong></div><div className="stat-card red"><span>Total expenses</span><strong>{money(totals.expenses)}</strong></div><div className="stat-card green"><span>Remaining budget</span><strong>{money(totals.budgetLeft)}</strong></div><div className="stat-card gold"><span>Pending revenue</span><strong>{money(totals.pending)}</strong></div></div></section> }
function Reports({ data, totals }) { return <section className="reports"><div className="panel report-cover"><div><p className="eyebrow">VIGNESHWARA TEAM · 2026</p><h2>Financial transparency report</h2><p>All figures are calculated from recorded income and expense entries.</p></div><button className="secondary">⇩ Export report</button></div><div className="report-cards"><div className="panel"><span>Total income</span><strong>{money(data.incomes.reduce((s, x) => s + Number(x.amount), 0))}</strong><small>Recorded revenue</small></div><div className="panel"><span>Total chandha</span><strong>{money(data.chandha.reduce((s, x) => s + Number(x.amount), 0))}</strong><small>{data.chandha.length} contributors</small></div><div className="panel"><span>Total expenses</span><strong>{money(totals.expenses)}</strong><small>{data.expenses.length} entries</small></div></div></section> }
