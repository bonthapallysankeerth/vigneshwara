...
create table if not exists public.loans (
  id uuid primary key default gen_random_uuid(), lender_name text not null,
  amount numeric(12,2) not null check (amount > 0), date date not null default current_date,
  note text, created_by uuid references auth.users(id), owner_id uuid,
  created_at timestamptz not null default now()
);

create table if not exists public.loan_repayments (
  id uuid primary key default gen_random_uuid(), loan_id uuid not null references public.loans(id) on delete cascade,
  amount numeric(12,2) not null check (amount > 0), date date not null default current_date,
  note text, created_by uuid references auth.users(id), owner_id uuid,
  created_at timestamptz not null default now()
);
...
alter table public.loans add column if not exists owner_id uuid;
alter table public.loan_repayments add column if not exists owner_id uuid;
...
begin
  ...
  if legacy_owner is not null then
    ...
    update public.loans set owner_id = coalesce(owner_id, created_by, legacy_owner) where owner_id is null;
    update public.loan_repayments set owner_id = coalesce(owner_id, created_by, legacy_owner) where owner_id is null;
  end if;
end $$;
...
drop policy if exists "authenticated users can read loan repayments" on public.loan_repayments;
drop policy if exists "admins can manage loan repayments" on public.loan_repayments;
...
create policy "authenticated users can read loans" on public.loans for select to authenticated using (owner_id = public.current_association_id());
create policy "admins can manage loans" on public.loans for all to authenticated using (owner_id = public.current_association_id() and public.is_admin()) with check (owner_id = public.current_association_id() and public.is_admin());
create policy "authenticated users can read loan repayments" on public.loan_repayments for select to authenticated using (owner_id = public.current_association_id());
create policy "admins can manage loan repayments" on public.loan_repayments for all to authenticated using (owner_id = public.current_association_id() and public.is_admin()) with check (owner_id = public.current_association_id() and public.is_admin());

do $$
declare
  target_table text;
begin
  foreach target_table in array array['chandha', 'expenses', 'sponsors', 'budget', 'festival_programs', 'bookings', 'loans', 'loan_repayments'] loop
    if not exists (
      select 1
